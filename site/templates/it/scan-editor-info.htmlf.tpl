<h2>Modifica la mappa</h2>
    
<p>
    Ci sono due modi per editare OpenStreetMap utilizzando questa 
		mappa come guida. Se preferisci usare l'editor
		OSM avanzato <a href="http://josm.openstreetmap.de">JOSM</a>,
		vai alla pagina <a href="http://wiki.openstreetmap.org/wiki/JOSM/Plugins/WalkingPapers">JOSM WalkingPapers Plugin</a>.
</p>

<p>
    Puoi anche editare OpenStreetMap direttamente in questa pagina usando l'editor OSM web-based
    <a href="http://wiki.openstreetmap.org/index.php/Potlatch">Potlatch</a>.
</p>
